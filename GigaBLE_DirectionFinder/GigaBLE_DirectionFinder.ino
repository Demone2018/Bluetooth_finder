...
void drawScanHeader() {
  ...
  // RUOTA button (always visible, top-left after title)
  drawBtn(220, 12, 50, 36, C_BTN_B, "⟲");
  ...
  if (usePortrait) {
    ...
    // UP button
    display.fillRoundRect(280, 12, 44, 36, 6, 0x2945);
    display.setTextColor(C_WHITE); display.setTextSize(2);
    display.setCursor(288, 20); display.print("UP");
    // DN button
    display.fillRoundRect(330, 12, 44, 36, 6, 0x2945);
    display.setCursor(338, 20); display.print("DN");
    // SCANSIONA button
    drawBtn(380, 12, 90, 36, C_BTN_G, "SCANSIONA");
  } else {
...
void handleTouch(int tx, int ty) {
  ...
  if (appState == SCAN_SCR) {
    // RUOTA button (rotation toggle) - always at same position
    if (ty>=12 && ty<48 && tx>=220 && tx<270) {
      Serial.println("[ACTION] RUOTA (rotation) clicked!");
    ...
    // SCANSIONA button
    int scanBtnX = usePortrait ? 380 : 650;
    int scanBtnY = 12;
    int scanBtnW = usePortrait ? 90 : 138;
    int scanBtnH = 36;
    ...
    // UP scroll button
    int upBtnX = usePortrait ? 280 : 548;
    if (ty>=12 && ty<48 && tx>=upBtnX && tx<upBtnX+44) {
      if (scrollOff>0) { scrollOff--; drawScanList(); } return;
    }

    // DN scroll button
    int dnBtnX = usePortrait ? 330 : 598;
    if (ty>=12 && ty<48 && tx>=dnBtnX && tx<dnBtnX+44) {
